...
为了构建可持续的价值循环，该项目的机制设计需要实施严格的链上流转管理。客户要求机制如下：
...
为什么说它们很难区分？我们看看 Uniswap/PancakeSwap V2 的对应处理。

在 Uniswap/PancakeSwap V2 的标准中，无论是 **“兑换（Swap）”还是“移除流动性（Burn）”**，对于 Token 合约来说，表现出的链上行为特征是完全一致的：

1.  **买入 (Swap)：** Pair 合约把 Token 转给用户。
2.  **撤池子 (Remove Liquidity)：** Pair 合约把 Token 转给用户。
...
* **状态瞬间：** 在本币发生转账的那一刻（`_transfer` 触发时），Pool 里的 USDT 余额 (Balance) 必然是显著增加的，甚至超过了它账本上记录的 储备量 (Reserve)。
* **形象比喻：** 水位超过了堤坝。
...
### 💡 于是发现区分办法
我们可以通过对比 **“配对 Token 在 Pool 中的实时余额”** 与 **“账本储备量”** 的关系，来精准识别买入行为！
